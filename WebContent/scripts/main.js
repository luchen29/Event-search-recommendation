(function() {
  ...
  function validateSession() {
    onSessionInvalid();
    // The request parameters到login这个endpoint
    var url = './login';
    ...
    //先默认用户是logout的； 发送一个ajax call给server验证用户身份是还是valid的
    // server端会验证cookie是否还存在；如果还存在的话回传200；否则回传403
    // make AJAX call
    ajax('GET', url, req,
      // session is still valid
      function(res) {
        var result = JSON.parse(res);
        //如果login成功的话，把不需要的部分进行隐藏
        if (result.status === 'OK') {
  ...
  function loadNearbyItems() {
    ...
    // The request parameters 到search这个endpoint
    var url = './search';
